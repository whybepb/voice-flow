...
export const getDashboardAnalytics = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const [bookings, callLogs, campaigns] = await Promise.all([
            ...
            prisma.callLog.findMany({ select: { callStatus: true, createdAt: true } }),
        ...
        const campaignCompleted = allCampaignBookings.filter(
            (b) => b.lastCallStatus?.toLowerCase() === 'completed'
        ).length;
        const campaignPending = allCampaignBookings.filter(
            (b) => b.status === 'PENDING'
        ).length;
        const campaignFailed = allCampaignBookings.filter(
            (b) => ['failed', 'busy', 'no-answer', 'canceled'].includes(b.lastCallStatus?.toLowerCase() || '')
        ).length;
