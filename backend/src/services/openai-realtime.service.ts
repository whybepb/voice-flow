...
const DEFAULT_AGENT_PROMPT =
  "You are the business's AI phone assistant. Help callers clearly, warmly, and efficiently.";
...
function normalizeVoiceMode(
  value?: VoiceModeValue | string | null,
): VoiceModeValue {
  return value === "PREMIUM" ? "PREMIUM" : "DEFAULT";
}

function normalizeVoiceOverride(value?: string | null): string | null {
  const trimmed = value?.trim();
  return trimmed ? trimmed : null;
}

export function composeSystemPrompt(editablePrompt?: string | null): string {
...
export function resolveRealtimeProfile(input: {
  voiceMode?: VoiceModeValue | string | null;
  userVoice?: string | null;
  voiceOverride?: string | null;
  userPrompt?: string | null;
}): RealtimeProfile {
  ...
  const fallbackVoice =
    voiceMode === "PREMIUM" ? PREMIUM_VOICE : DEFAULT_VOICE;
  const voice =
    normalizeVoiceOverride(input.voiceOverride) ||
...
const TOOLS = [
  {
    ...
    description:
      "Look up a customer's booking by phone number. Use this after telling the caller you are checking. Returns booking details including date, service, and status.",
    parameters: {
      type: "object",
      properties: {
        phone: {
  ...
  },
  {
    ...
    description:
      "Reschedule a booking to a new ISO 8601 date and time. Only call this after the caller explicitly confirms the exact new time.",
    parameters: {
      type: "object",
      properties: {
        booking_id: {
        ...
        new_date_time: {
          type: "string",
          description:
            "The new appointment date and time in ISO 8601 format, e.g. 2026-03-15T14:00:00Z",
        },
        confirmed: {
          type: "boolean",
          description:
            "Set to true only after the caller explicitly confirms the exact new appointment time.",
        },
      },
  ...
  },
  {
    ...
    name: "cancel_booking",
    description:
      "Cancel a booking. Only call this after the caller explicitly confirms that they want to cancel it.",
    parameters: {
      type: "object",
      properties: {
        booking_id: {
        ...
        confirmed: {
          type: "boolean",
          description:
            "Set to true only after the caller explicitly confirms they want to cancel the booking.",
        },
      },
  ...
  },
  {
    ...
    description:
      "Search the company knowledge base for services, policies, pricing, hours, FAQs, or other company-specific questions. Use this after telling the caller you are pulling it up.",
    parameters: {
      type: "object",
      properties: {
        query: {
          type: "string",
          description:
            "The search query based on what the caller is asking about",
        },
      },
      required: ["query"],
    },
...
function buildToolFollowupInstruction(toolName: string): string | null {
  switch (toolName) {
    ...
    case "check_booking_status": {
      if (!userId) {
        return JSON.stringify({
          found: false,
          message:
            "Unable to check bookings because no account context is available. Offer a human handoff.",
        });
      }
    ...
    case "reschedule_booking": {
      if (!userId) {
        return JSON.stringify({
          success: false,
          error:
            "Unable to reschedule because no account context is available. Offer a human handoff.",
        });
      }
      ...
      if (!args.confirmed) {
        return JSON.stringify({
          ...
          error:
            "The caller has not explicitly confirmed the new appointment time yet.",
        });
      }
    ...
    case "cancel_booking": {
      if (!userId) {
        return JSON.stringify({
          success: false,
          error:
            "Unable to cancel because no account context is available. Offer a human handoff.",
        });
      }
      ...
      if (!args.confirmed) {
        return JSON.stringify({
          ...
          confirmation_required: true,
          error:
            "The caller has not explicitly confirmed the cancellation yet.",
        });
      }
    ...
    case "search_knowledge": {
      if (!userId) {
        return JSON.stringify({
          found: false,
          message:
            "Unable to search the knowledge base because no account context is available. Offer a human handoff.",
        });
      }

      try {
        const results = await searchKnowledge(userId, args.query, 3);

        if (results.length === 0) {
          return JSON.stringify({
            found: false,
            message:
              "No relevant information was found in the knowledge base. Offer a human handoff.",
          });
        }

        const ragContext = buildRAGPrompt(args.query, results);
        return JSON.stringify({
          found: true,
        ...
        return JSON.stringify({
          found: false,
          message:
            "Knowledge base search failed. Offer a human handoff.",
        });
      }
    }
...
async function resolveCallContext(
  ...
  if (callerPhone) {
    ...
    if (customer?.userId) {
      console.log(
        `[RAG] Resolved userId ${customer.userId} from caller phone ${callerPhone}`,
      );
      return {
  ...
  console.warn(
    "[RAG] Could not resolve account context for this call. Realtime tools will be limited.",
  );
  return {
    voiceMode: "DEFAULT",
  };
...
function connectToOpenAI(apiKey: string, model: string): WebSocket | null {
...
export function handleMediaStream(twilioWs: WebSocket, callSid?: string) {
  console.log(
    `[MediaStream] New connection established${callSid ? ` for call ${callSid}` : ""}`,
  );

  let openaiWs: WebSocket | null = null;
  let streamSid: string | null = null;
  let callMetadata: { from?: string; to?: string } = {};
  let resolvedContext: ResolvedCallContext = { voiceMode: "DEFAULT" };
  const transcriptParts: string[] = [];
  const pendingAudioFrames: string[] = [];
  let isCleanedUp = false;

  let resolveOpenAIWsOpen: (() => void) | undefined;
  let rejectOpenAIWsOpen: ((error: Error) => void) | undefined;
  let openaiWsIsReady = false;
  const openaiWsOpenReady = new Promise<void>((resolve, reject) => {
  ...
  function attachOpenAIHandlers(ws: WebSocket) {
    ws.on("open", () => {
                  ...
                  try {
                    ...
                    if (parsed?.found && parsed?.answer_context) {
                      const sourceFiles = Array.isArray(parsed.sources)
                        ? parsed.sources
                            .map((source: { file?: string }) => source.file)
                            .filter(Boolean)
                            .join(", ")
                        : "";
                      safeSend(
                            ...
                            content: [
                              {
                                type: "input_text",
                                text:
                                  "Use the following knowledge base context to answer the caller. " +
    ...
    ws.on("error", (error) => {
      ...
      if (!openaiWsIsReady) {
        rejectOpenAIWsOpen?.(
          error instanceof Error ? error : new Error("OpenAI websocket error"),
        );
      }
    });
  }
    ...
    try {
      ...
      switch (data.event) {
        ...
        case "start":
          ...
          console.log(
            `[Twilio] Stream started: ${streamSid}${callSid ? ` (Call SID: ${callSid})` : ""}`,
          );
          ...
          resolveCallContext(callSid, callMetadata.to, callMetadata.from)
            .then(async (context) => {
              ...
              const apiKey = decryptSecret(context.openaiApiKey);
              if (!apiKey) {
...
async function saveTranscript(callSid: string | undefined, transcript: string) {
  ...
  try {
    ...
    if (callLog) {
      ...
      console.log(
        `[Transcript] Saved and queued post-call analysis for ${callSid}`,
      );
    } else {
