...
const TOOLS = [
  {
    ...
    description:
      "Look up a customer's booking by their phone number. Returns booking details including date, service, and status.",
    parameters: {
      type: "object",
      properties: {
        phone: {
  ...
  },
  {
    ...
    description:
      "Search the company knowledge base for information about services, policies, pricing, hours, FAQs, or anything company-specific. Use this when the caller asks a question that might be answered by company documentation.",
    parameters: {
      type: "object",
      properties: {
        query: {
    ...
    case "check_booking_status": {
      if (!userId) {
        return JSON.stringify({
          found: false,
          message:
            "Unable to check bookings — no user context available. Please offer to connect the caller with a human agent.",
        });
      }
    ...
    case "reschedule_booking": {
      if (!userId) {
        return JSON.stringify({
          success: false,
          error:
            "Unable to reschedule — no user context available. Please offer to connect the caller with a human agent.",
        });
      }
      try {
    ...
    case "cancel_booking": {
      if (!userId) {
        return JSON.stringify({
          success: false,
          error:
            "Unable to cancel — no user context available. Please offer to connect the caller with a human agent.",
        });
      }
      try {
        const result = await prisma.booking.updateMany({
    ...
    case "search_knowledge": {
      if (!userId) {
        return JSON.stringify({
          found: false,
          message:
            "Unable to search knowledge base — no user context available. Please offer to connect the caller with a human agent.",
        });
      }
      ...
      try {
        ...
        if (results.length === 0) {
          return JSON.stringify({
            found: false,
            message:
              "No relevant information found in the knowledge base. Suggest transferring to a human agent who might be able to help.",
          });
        }
        ...
        return JSON.stringify({
          found: false,
          message:
            "Knowledge base search encountered an error. Offer to connect the caller with a human agent.",
        });
      }
    }
...
async function resolveUserId(
  ...
  if (callSid) {
    ...
    if (callLog?.userId) {
      console.log(
        `[RAG] Resolved userId ${callLog.userId} from callSid ${callSid}`,
      );
  ...
  if (calledPhone) {
    ...
    if (user?.id) {
      console.log(
        `[RAG] Resolved userId ${user.id} from called phone ${calledPhone}`,
      );
      return user.id;
    }
  }
  ...
  console.warn(
    "[RAG] Could not resolve userId for this call — RAG search will be disabled",
  );
  return undefined;
}
...
function connectToOpenAI(apiKey: string, retryCount = 0): WebSocket | null {
  ...
  try {
    ...
    console.error(
      `[OpenAI] Connection attempt ${retryCount + 1} failed:`,
      error,
    );
    return null;
  }
...
export function handleMediaStream(twilioWs: WebSocket, callSid?: string) {
  ...
  let resolveOpenAIWsOpen: () => void;
  const openaiWsOpenReady = new Promise<void>((resolve) => {
