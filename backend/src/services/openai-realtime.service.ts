 ...
 */
async function resolveUserId(callSid?: string, calledPhone?: string, callerPhone?: string): Promise<string | undefined> {
        ...
        try {
            ...
            switch (data.event) {
                ...
                case 'start':
                    ...
                    openaiReady.then(() => {
                        if (isCleanedUp) return; // call may have ended while we waited
                        const initialEvent = {
                            type: 'conversation.item.create',
                            item: {
                                type: 'message',
